function [V, mapMatrix] = TrustRankSparseOptimize()
% @return: 
format long
% Data info:
% filename: data.txt
...
mapMatrix = ones(1, matrixSize);
for index = 1 : matrixSize
    mapMatrix(1, index) = index - 1;
end
...
while true
    flag = false;
    for index = 1 : col % Traverse the whole matrix
        while index <= col && sum(S(:, index)) == 0 % If dead end found
            S(:, index) = []; % Remove the column
            S(index, :) = []; % Remove the row
            mapMatrix(:, index) = []; % Remove mapping
            col = col - 1; % size--
            flag = true; % Something has changed
        end
    end
    if ~flag
        break;
